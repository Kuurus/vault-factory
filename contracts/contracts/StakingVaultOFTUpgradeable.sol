...
import "./interfaces/IETFVaultUpgradeable.sol";
import "./interfaces/IWithdrawalHandler.sol";

// Separate storage contract to avoid storage collisions in upgrades
abstract contract ETFVaultStorageV1 {
    bytes32 internal constant ETF_VAULT_STORAGE_POSITION = keccak256("ETFVault.storage.location");

    struct ETFVaultStorage {
        mapping(address => bool) blacklist;
        address withdrawalHandler;
    }

    function getETFVaultStorage() internal pure returns (ETFVaultStorage storage s) {
        bytes32 position = ETF_VAULT_STORAGE_POSITION;
        assembly {
            s.slot := position
        }
    }
    ...
    ETFVaultStorageV1
{
    ...
    function blacklistAccount(address account) external onlyRole(BLACKLIST_MANAGER_ROLE) {
        ETFVaultStorage storage s = getETFVaultStorage();
