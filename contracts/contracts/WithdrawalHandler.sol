...
contract WithdrawalHandler is AccessControl, ReentrancyGuard {
    ...
    bytes32 public constant ETF_VAULT_ROLE = keccak256("ETF_VAULT_ROLE");

    IERC20 public immutable etf;
    ...
    constructor(address _etf, uint256 _withdrawPeriod) {
        etf = IERC20(_etf);
        withdrawPeriod = _withdrawPeriod;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function createWithdrawalRequest(address user, uint256 amount) external onlyRole(ETF_VAULT_ROLE) returns (uint256) {
    ...
    function claimWithdrawal(uint256 requestId) external nonReentrant {
        ...
        etf.safeTransfer(msg.sender, request.amount);
